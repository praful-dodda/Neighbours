function [psub,zsub,dsub,nsub,index]=neighbours_stg(p0,data_stg,nmax,dmax)
    ...
    % SYNTAX :
    %
    % [psub,zsub,dsub,nsub,index]=neighbours_stg(p0,data_stg,nmax,dmax);
    %
    % INPUT :
    %
    ...
    %                  Hence, the fields for data_stg that are needed are
    %                    data_stg.Zisnotnan   nMS by nME logical array, true if Z is not a nan
    %                    data_stg.sMS  nMS by 2  spatial coordinates of nMS Monitoring Stations (MS)
    %                    data_stg.tME  1 by nME  time of nME Monitoring Events (ME)
    %                    data_stg.nanratio  scalar  fraction of Z values that are nan
    %                    data_stg.index_stg_to_stv  nMS*nME by 1 vector converting 
    %                      an index_stg for z_stg to an index for z. It can be
    %                      calcated as data_stg.index_stg_to_stv=cumsum(data_stg.Zisnotnan(:))
    %                    data_stg.p  n by 3  matrix of the space/time coordinate of z, where n<=nMS*nME
    %                    data_stg.z  n by 1  vector of the values z
    ...
    % nMS=10;
    % nME=5;
    % data_stg.Z=rand(10,5);
    % data_stg.Z(1:3:50)=nan;
    % data_stg.sMS=rand(nMS,2);
    % data_stg.tME=1:nME;
    % data_stg.Zisnotnan=~isnan(data_stg.Z);
    % data_stg.nanratio=sum(~data_stg.Zisnotnan(:))/length(data_stg.Zisnotnan(:));
    % data_stg.index_stg_to_stv=cumsum(data_stg.Zisnotnan(:));
    % [p_stg,z_stg]=valstg2stv(data_stg.Z,data_stg.sMS,data_stg.tME);
    % data_stg.p=p_stg(~isnan(z_stg),:);
    % data_stg.z=z_stg(~isnan(z_stg));
    % nmax=5;
    % dmax=[100 100 0.1];
    % [psub,zsub,dsub,nsub,index_stg]=neighbours_stg(p0,data_stg,nmax,dmax);
    
    nMS=size(data_stg.sMS,1); % number of MS
    nME=size(data_stg.tME,2); % number of ME
    
    if isempty(data_stg.z)
    ...
    dMSsub=coord2dist(data_stg.sMS,s0);        %   dMSsub = distance between MS subset and s0
    
    indexMEsub=(1:nME);               %   indexMEsub : index of ME subset
    dMEsub=abs(data_stg.tME-t0);               %   dMEsub : time difference between ME subset and t0
    ...
    % Hence we retain at least nmax/(1-data_stg.nanratio). Since we need whole 
    % number, we retain ceil(nmax/(1-data_stg.nanratio)) MS closest to s0.
    % 
    ...
    nMSsubReduced=nMSsub;             % keep at most ceil(nmax/(1-data_stg.nanratio)) MS
    if ceil(nmax/(1-data_stg.nanratio))<nMSsubReduced
      nMSsubReduced=ceil(nmax/(1-data_stg.nanratio));
    end
    ...
    nsubReduced=sum(reshape(data_stg.Zisnotnan(indexMSsubReduced,indexMEsubReduced),...
    ...
    if nsubReduced<nmax  && (nMSsubReduced<nMSsub || nMEsubReduced<nMEsub)
      ...
      while nsubReduced<nmax && ( ii+1<=length(nMSsubCandidates) || jj+1<=length(nMEsubCandidates) )
        ...
        nsubReduced=sum(reshape(data_stg.Zisnotnan(indexMSsubReduced,indexMEsubReduced),nMSsubReduced*nMEsubReduced,1));
      end
    ...
    indexZsubIsNotNan=reshape(data_stg.Zisnotnan(indexMSsub,indexMEsub),nMSsub*nMEsub,1);
    ...
    index=data_stg.index_stg_to_stv(index_stg);
    nsub=length(index);
    
    % extract the subset p and z
    psub=data_stg.p(index,:);                  % extract the subset p
    zsub=data_stg.z(index);                    % extract the subset z
